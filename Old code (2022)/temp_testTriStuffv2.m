...
t_abs_mr = zeros(size(t_oth));

prettycolors = { 'r' 'g' 'c' 'k' 'm' 'y' };
...
%intersect = TriangleRayIntersection(orig, dir, vert1, vert2, vert3);
    ...
    [intersect, t, ~, ~] = TriangleRayIntersectionMK( ar_xyz , t_refln(:,ti)', t_vert0, t_edg1, t_edg2, 'border','exclusive' );
%  intersect = TriangleRayIntersection( t_cxyz, t_refln', t_naxyz, t_nbxyz, t_ncxyz );
    ...
    if myhitts 
        ...
        while 1
            ...
            if ar_costheta < 0, warning('Negative angle!'), break, end
            ...
            [intersect, t, ~, ~] = TriangleRayIntersectionMK( [t_cx(myhitts(1)) t_cy(myhitts(1)) t_cz(myhitts(1))] ,...
                                    rr_dir', t_vert0, t_edg1, t_edg2, 'border','exclusive' );   % this version traces from centroids
