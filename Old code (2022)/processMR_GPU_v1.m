...
if 1
    ...
    thisseemsdumb = 1:numtris;      %fixme, move to main sim code
...
%    anybroken = 1;
    ...
    if anybroken
        ...
        %     gpu_ox =       gpuArray(t_cx(t_notbroken));  %TODO:  change to gpu_ray_oxyzs
        %     gpu_oy =       gpuArray(t_cy(t_notbroken));
        %     gpu_oz =       gpuArray(t_cz(t_notbroken));
    else
        ...
        %     gpu_ox =       gpuArray(t_cx);  % varies %TODO:  change to gpu_ray_oxyzs, correct orientation, ...
        %     gpu_oy =       gpuArray(t_cy);  % varies
        %     gpu_oz =       gpuArray(t_cz);  % varies
    end
...
%         gpu_ray_dirs = [gpu_dx; gpu_dy; gpu_dz];  % FIXME this is temporary, for debugging
        ...
        while 1
            ...
            if (ri >= MAX_REFL )
                ...
                if ~showplots, gpu_dest_xyzs = gpu_oxyzs(:,gpu_m) + gpu_hit_t.* gpu_ray_dirs(:,gpu_m); end
            ...
            if useSpecularTMM  % now calculate optical response and assign forces/power to triangles
                ar_LULi = max(min(floor(Result_theta ./ LULAngleStep)+1,LUL.num),1);
