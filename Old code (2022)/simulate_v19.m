...
simdesc = 'comParab_16'; % Use this if you want to add a short description of why you're running the simulation.  
...
useSpecularTMM = 1;
LULAngleStepDeg = 0.5; % angular resolution of LUL.  Don't comment this out, even if not using SpecularTMM!
...
ReverseRayTrace = 1;  % whether or not reverse raytracing is used.  Only produces valid results for a tethered payload.
...
outersweepvals1 = [0.001 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5 ];  % currently used for:  Tethered payload mass ratio 
burstTestVals = [];
...
for nouterloop1 = 1:length(outersweepvals1)
    for nouterloop2 = 1:length(outersweepvals2)
        ...
        %% Generate mesh
        generateMesh_v19; % This helper script sets up the mesh.  Modify it to change mesh settings such as shape, patterning, and material properties.
        ...
        Irad = radiusmm/2; % * 0.5 * sqrt(2); %radiusmm * outersweepvals2(nouterloop2);  % mm, beam waist radius, for gaussian illumination
        ...
        rps_target = 250;%outersweepval2; % Rotations per second for spin stabilization.  Can be negative to spin the other way.
        ...
        t_ramp_delay = 1e-2; % (seconds), time delay before starting beam turn-on, formerly t_start_ramp
        ...
        t_on_dur = 0.001;%00e-3; % (seconds) duration of acceleration phase, after which beam is turned off
        t_coast_dur = 0.001; % (seconds) duration of time after beam turn-off for observing the final sail trajectory.
        ...
        t0t_changeSpinAngle = 0; % whether or not to change the rotational position of the sail, useful for spinning sails
        ...
        t0t_x0 = 0.05*radiusmm;
        t0t_y0 = 0.05*radiusmm;
        t0t_t0 = 0; % (rad) initial tilt of sail.  Tilt rotation is performed about the x axis.  Not yet supported.
        ...
        sim_output_downsample = 32;  % typical recommended value: 8
        ...
        opt_skip_downsample=8;  % number of time steps between optical force calculation.  Do not set to zero!
        ...
        breakup_slowmo_interval_s = frame_interval_s;% max( frame_interval_s/10, 5e-6);   %seconds; increases frame rate 10x for failures
        ...
        snapshot_interval_s = 0.05;%0.020;%.250;
        ...
        if (videoMode==1)
            ...
            fSimVid = figure('pos',[100        100        1280         1024]);  % for my laptop!
        end
        if videoMode==0
