...
% 01/23/2022: Corrected mistakes in how sail is rotated, its initial
% velocity vector is transformed, and how Euler angles are calculated
...
simdesc = 'flexM1e120Hz0p4D5p5p0d0d0Abs'; % Use this if you want to add a short description of why you're running the simulation.  
...
setupMaterialProperties_v19_final2_0Absorption_RG;  
...
spin_frequency = 120; % in Hz

% outersweepvals2 = 500 ; %fliplr(50 * 10.^[1.6:0.025:2]);  % currently used for:  radius
outersweepvals2 = [ [beam_width, spin_frequency] ]; % sweeping beam width & spin_frequency simultaneously
% outersweepvals1 = [1000]; %currently used for:  power
...
%         outersweepval2 = outersweepvals2(nouterloop2);
        ...
        %% Generate mesh
        generateMesh_v19_medium_RG; % This helper script sets up the mesh.  Modify it to change mesh settings such as shape, patterning, and material properties.
...
%         I0 = outersweepval1; % W/mm^2, peak incident power density (1000 = 1GW/m2).
        I0 = 1000; % W/mm^2, peak incident power density (1000 = 1GW/m2).
        
        %% Illumination settings for gaussian beam profile.
        Irad = (outersweepval2(1)) * sqrt(2) * radiusmm; % * 0.5 * sqrt(2); %radiusmm * outersweepvals2(nouterloop2);  % mm, beam waist radius, for gaussian illumination
...
%         t_ramp_delay = 1e-3; % (seconds), time delay before starting beam turn-on, formerly t_start_ramp
        t_ramp_delay = 0*1e-3; % (seconds), time delay before starting beam turn-on, formerly t_start_ramp
%         t_ramp_dur = 4e-3;%20;%3e-3; % (seconds), time duration of beam turn-on. DO NOT SET TO ZERO!  (Use a small number e.g., 1e-8 for instant turn-on.)
        ...
        t_on_dur = 5.01;%00e-3; % (seconds) duration of acceleration phase, after which beam is turned off
...
%         t0t_x0 = 0;%0.2*radiusmm;
%         t0t_y0 = 0;%0.2*radiusmm;
...
%                              % smart enough to offer tilt around the y axis as well.
        ...
        t0t_x0 = 0.1*radiusmm;
        t0t_y0 = 0.1*radiusmm;
        t0t_t0_x = deg2rad(0); % (rad), initial tilt of sail about the x axis
        t0t_t0_y = deg2rad(0); % (rad), initial tilt of sail about the y axis
...
%         frame_interval_s = 0.5*1e-4;     %seconds
...
%         snapshot_interval_s = 0.5;%0.020;%.250;
        
        num_Snapshots = ceil(t_on_dur/snapshot_interval_s) + 1;
...
%         colorbar ;
        ...
        tab_yaw = zeros(max(size(t_a)),sim_dur_nto2);  % histogrammed yaw angles
        ...
        snapshots_TRI = zeros(num_Snapshots,size(TRI,1),size(TRI,2));
        snapshots_constrainedEdges = zeros(num_Snapshots,size(constrainedEdges,1),size(constrainedEdges,2));

        snapshots_t_broken = zeros(num_Snapshots,size(t_broken,2));

        snapshots_n_x = zeros(num_Snapshots,size(n_x,2));
        snapshots_n_y = zeros(num_Snapshots,size(n_y,2));
        snapshots_n_z = zeros(num_Snapshots,size(n_z,2));

        snapshots_t_t = zeros(num_Snapshots,size(t_t,2));

        snapshots_comz = zeros(num_Snapshots,size(comz,2));

        snapshots_pitch_angles = zeros(num_Snapshots,numtris);
        snapshots_roll_angles = zeros(num_Snapshots,numtris);
        snapshots_yaw_angles = zeros(num_Snapshots,numtris);
...
%disp('Done with intensity');
    ...
    %             disp('got norms');
            ...
            elseif ~read_LUT
                t_norms = cross( t_ev2, t_ev1 );  % for propulsion in -z - sorry, I (Ramon) am still not smart enough to figure out correct calculation of forces in +z
...
%disp('Done with specular');
                ...
                %% 3b. Calculate per-triangle incident power, specular optical forces, & absorption heat-loading
...
%             end
            ...
            %% 4. Do RayTracingMode and/or non-specular optics  
...
%                 end
    ...
    %                 end
            ...
            elseif ~read_LUT
            %% 4c. Calculate optical response of non-specular ('textured') regions with Ramon's slow approach  
            ...
            %% 6. Calculate thermal emission (radiatively emitted power)
...
%             n_ems = SBCmm .* n_a .* Emissivity .* (n_t.^4); % Watt
            ...
            %% 7. Calculate mechanical forces & heat conduction
            ...
            %% 8. Assign mechanical forces to nodes (from edges) and also heat flow
            ...
            %% 9. Evaluate for tensile or thermal failure
...
%                 end
            ...
            %% 10. Mechanical calculations and time stepping
...
%                 end
            ...
            %% 13. Update total body velocity, KE, spin speed...
            ...
            %% 14. Update inertia tensors for rigid bodies
...
%                                     cdata = print(gcf, '-RGBImage');
                    ...
                    if ((nt-ps >= snapshot_intvl-1 ) && (ns < 50) ) || (graphuos(nto) >= nextExplosionSnapshot) || ( (tt >= 0) && save_first_snapshot == 1)
                        ...
                        snapshots_TRI(ns,:,:) = TRI;
                        snapshots_constrainedEdges(ns,:,:) = constrainedEdges;
                        
                        snapshots_t_broken(ns,:) = t_broken;
                        
                        snapshots_n_x(ns,:) = n_x;
                        snapshots_n_y(ns,:) = n_y;
                        snapshots_n_z(ns,:) = n_z;

                        snapshots_comz(ns) = comz;

                        snapshots_t_t(ns,:) = t_t;

                        snapshots_pitch_angles(ns,:) = pitch_angles;
                        snapshots_roll_angles(ns,:) = roll_angles;
                        snapshots_yaw_angles(ns,:) = yaw_angles;
...
%                         save([filebasename '_workspace',num2str(ns-1),'.mat'],  allvars_snapshot(tosave_snapshot).name, '-v7.3');
...
%         tosave = cellfun(@isempty, regexp({allvars.class}, '^matlab\.(ui|graphics)\.'));
        ...
        % Pass these variable names to save
        save([filebasename '_workspace.mat'],  allvars(tosave).name, '-v7.3');
