...
simdesc = 'parFlex_RT'; % Use this if you want to add a short description of why you're running the simulation.  
...
% based on 'outerloop' variables, then re-invoke it within each loop, e.g., near generateMesh.
...
outersweepvals2 = [40];  % currently used for:  spin speed
outersweepvals1 = [1 0]; %currently used for:  raytrace_mode
...
for nouterloop1 = 1:length(outersweepvals1)
    for nouterloop2 = 1:length(outersweepvals2)
        ...
        %% Generate mesh
        generateMesh_v17; % This helper script sets up the mesh.  Modify it to change mesh settings such as shape, patterning, and material properties.
        ...
        videoMode = 0;  %0 = full plots; 1 = simplified plot (slightly faster movie rendering); 2 = no plots (no video; fastest option)
        ...
        breakup_ratio = 2;  % by how many times the initial sail radius the tattered remains of the sail can extend (radially) before the simulation stops.
        ...
        termination_delay_s = 0.0500; %seconds of additional simulation time for delayed termination.
        ...
        t_aproj = t_a;  % triangle area in light incidence plane, i.e., projected area in xy plane
        ...
        e_t = (zeros(size(e_na))); 
        n_a = (zeros(size(n_x)));  % mm2, effective radiation area for each node, for radiative cooling calcs         
        ...
        timers = zeros(1,14);  % this is where we keep track of the time spent on various simulation calcuation steps, for
        ...
        while nt<=sim_dur_nt 
            ...
            upsidedowntris(nt) = sum( t_norms(3,:) < 0);  % we determine if a triangle is upside down based on the z 
...
%              db_bm_timer_itr = db_bm_timer_itr + db_bm_timer_itr_val;
                ...
                timers(13) = timers(13) + toc(timerStartRT); % the RT code sometimes calls tic() on its own, so need to explicitly reference the toc() to the correct time.
                tic;
...
%                 end
    ...
    %                 end
            ...
            e_hf = Thermcondmm .* e_al .* e_dt;  % Watts
            ...
            if ~rigid_mode  % we skip tensile failure analysis in rigid mode, since the mesh is rigid.
                ...
                e_nowbroken =  ( e_broken | ( e_s > tensile_strain_limit ));  %all edges that are currently or were previously broken
                e_newbroken = xor( e_nowbroken, e_broken); % boolean xor identifies newly broken edges... so far only used for debugging
...
%                 end
            ...
            if rigid_mode  % rigid body dynamics
                ...
                %if switching from flexible to rigid dynamics, angVelRigid will be 0.  If that's the case, we'll use the
                % approximate angular velocity we calculated from the mesh as our starting rigid angular velocity.
                % Otherwise, we'll use the previous value of angVelRigid, since I haven't figured out how to correctly
                % calculate the angular velocity from the mesh alone.
...
%                 fprintf('\n');
                ...
                n_dxyz_new = ihaterotationmatrices * [ n_dxyz ];
            ...
            % keep track of time spent on mechanical calcs
            ...
            fpmsucksthreshold = 0.01*radiusmm;
            ...
            if ( ~mod(nt, sim_output_downsample) )
                ...
                vertzs(nto) = n_z(1);
                if (rigid_mode)
                    angVels(:,nto) = angVelRigid;
                else
                    angVels(:,nto) = angVel;
                end
                angVelsRigid(:,nto) = angVelRigid;
                torques(:,nto) = rigidTorque;
                spinAngs(nto) = approxSpinAng;  %disp([num2str(rad2deg(approxSpinAng)) ' sam_wait=' num2str(sam_wait) ] );
                sailNorms(:,nto) = mean(t_norms(:,n_centerTris),2);
                sailPitches(:,nto) = atan2( sqrt(sailNorms(1,nto).^2 + sailNorms(2,nto).^2), sailNorms(3,nto) );
                ofxs(nto) = rigidForceX;
                ofys(nto) = rigidForceY;
                ofzs(nto) = rigidForceZ;
                nofxs(nto) = rigidForceX / (I0 * ((2*radiusmm).^2) / c0);
                nofys(nto) = rigidForceY / (I0 * ((2*radiusmm).^2) / c0);
                nofzs(nto) = rigidForceZ / (I0 * ((2*radiusmm).^2) / c0);
                tqxs(nto) = rigidTorqueX;
                tqys(nto) = rigidTorqueY;
                tqzs(nto) = rigidTorqueZ;
                ntqxs(nto) = rigidTorqueX / (I0 * ((2*radiusmm).^3) / c0);
                ntqys(nto) = rigidTorqueY / (I0 * ((2*radiusmm).^3) / c0);
                ntqzs(nto) = rigidTorqueZ / (I0 * ((2*radiusmm).^3) / c0);
                graphuos(nto) = uhohs;
                graphts(nto) = tt;
            ...
            % keep track of time spent updating the mesh and recording monitor data
            timers(9) = timers(9) + toc;
            tic;
            
            %% 17.  Display plot for movie:
            if (nt - pf >= frame_interval - 1)
                ...
                stringtodisplay = ['n = ' num2str(nt) ',  nto = ' num2str(nto) ',  nf = ' num2str(nf) ',  tt = ' num2str(tt)  ...
                    ...
                    ',  x = ' num2str(comx) ',  y = ' num2str(comy) ',  z = ' num2str(comz) ...
                    ',  max_str = ' num2str(max_strain) ',  maxv = ' num2str(maxvrel) ...
            ...
            if ((nt-ps >= snapshot_intvl-1 ) && (ns < 10) ) || (uhohs >= nextExplosionSnapshot)
                ps=nt+1;
                ns=ns+1;
            ...
            %nto>749 ||
            if( usercancel || (max(n_vx) > 1e12) || (xextentratio > breakup_ratio ) || ...
        ...
        et=toc(startsimtic);
        rate_oa = nt/et;
        sim_time = sum(timers([1:9 12 14]));
        render_time = sum(timers([ 10 11] ));
        raytrace_time = sum(timers(13));
