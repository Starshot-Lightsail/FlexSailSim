...
% efficient way during simulations.  While the optical response of simple surfaces, e.g., homogeneous isotropic
...
% be concatenated in order to produce a single listing of all simulation
...
% interest.  However, the following ordering is required for the concatenated table:
...
%   Note: 2D linear interpolation will be used to construct the LUTs from the source data.
...
% Our simulator is designed to simulate flat or topologically flattenable lightsail shapes only.  An implicit assumption
