...
if (radialRings > showMeshCutoff)
    if colormode < 2
        ...
        trisurf(TRI(goodTs',:),n_x,n_y,zreliefmag*n_z,  n_t, 'FaceColor', 'interp', 'LineStyle','none','FaceLighting','Gouraud','AmbientStrength',0.5,'SpecularStrength',0.3,'DiffuseStrength',0.5);
    end
   ...
   %view(0,0);
    ...
    if colormode<2
        ...
        trisurf(TRI(goodTs',:),n_x,n_y,zreliefmag*n_z,  n_t,'FaceColor','interp','FaceLighting','Gouraud','AmbientStrength',0.5,'SpecularStrength',0.3,'DiffuseStrength',0.5);
    end
...
if ReverseRayTrace && (nto > 1) 
    
     %   quiver3( t_cx, t_cy, t_cz, RRT_odirs(1,:), RRT_odirs(2,:), RRT_odirs(3,:), 3);
end
