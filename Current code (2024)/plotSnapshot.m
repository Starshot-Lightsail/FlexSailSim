%if (plotFast < 2)
    if (radius > 7)
        ...
        trisurf(TRI(~t_broken,:),n_x,n_y,n_z-comz+(ns-1)*xbox*2, n_t, 'FaceColor', 'interp' ,'LineStyle','none','FaceLighting','Gouraud','AmbientStrength',0.5,'SpecularStrength',0.5,'DiffuseStrength',0.5);
        plotMeshOutlineZ0=comz-(ns-1)*xbox*2;
        plotMeshOutline;
    else
        trisurf(TRI(~t_broken,:),n_x,n_y,n_z-comz+(ns-1)*xbox*2, n_t, 'FaceColor', 'interp');%,'LineStyle','none');
        hold on;
