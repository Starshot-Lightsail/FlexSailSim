...
% meshed.  At most a full hemisphere could be meshed, but even this seems inadvisable wihtout improvements to the code.
% 
% Ultimately the simulation is currently suitable only for flat or modestly curved membrane shapes.  An implicit assumption
% is that  lightsail membranes are infinitesimally thin and thus have negligable (zero) out-of-plane bending stiffness.  
% If you wanted to simulate a structure with out-of plane stiffness (other than a rigid body), you could implement appropriate 
% mechanics in the membrane simulation code (to calculate bending moment and torque), or alternatly, develop a 3D meshing 
% topology to simulate multi-layer meshes while also altering the optical and thermal physics calculations to deal with the 
% new topology.  As of 2023, the we do not calculate occlusion (self-shading) -- we assume all triangles are illuminated at 
% all times
%
% How to use:  This script is called automatically near the beginning of each simulation.  You don't need to run it on your
% own, but you can, for example, to try out new geometry settings and make sure the mesh is correct before a simulation.
%
% All user-changable settings for this script are near the top, and well documented.  Here, you can change the basic geometry
% (shape, size, etc.), the fidelity of mesh, and the assignment (mapping) of material properties.
%
% Tip:  This generator script can be run on its own and will produce a plot of the resulting mesh.  This is
% the best way to figure out which settings to use to get the desired structure for simulation.
% HOWEVER:  This script requires certain setting varialbles and data tables to exist in memory, prior to being invoked.
% If you are starting a fesh MATLAB session, it's easiest to just run the main simulator script, then abort that simulation,
% before trying to run this script independently to visualize and adjust the simulation structure.  Once the desired settings
% are obtained here, you can go back and start a new simulation.
%
% Latest changes:
% V18 -- support for initial "payload" mass distributions and edge tethers
% V19 -- support for partial rigid bodies (beta)
% V20 -- streamlined settings, better support for optiacal models and mapping

disp('Sharshot lightsail simulator mesh generator version 20 (c) 2019-2023 Michael Kelzenberg, Ramon Gao, California Institute of Technology');
...
if exist('filebasename','var')  % 'filebasename' should be an idenifying string with which we can name the saved file, and 
    %should have been uniquely generated by the main simulator script, which invokes this mesh generator script.  
    % We'll use this to save a copy of the present meshing script, unless a file already exists with that name, 
    % in which case we'll just warn the console.  (This happens when I'm adjusting and testing the mesh generator 
    % settings, between actual simulation attempts.)
    ...
    warning(['It looks like the mesh generator is being run outside of the simulation script... if you get errors, try starting a simulation first, ' ...
        ' to properly configure the workspace.  You can cancel it, then come back to the mesh generator and run it to test your settings.']);
end
...
radiusmm = 500;%710/2;  % (mm) the nominal radius (or inner half-width) of the lightsail.  NOTE:  due to the way I've 
  ...
  % tends to produce meshes of approximately similar mass despite adjusting zAspectRatio.
  %
  % HOWEVER, if you want to ensure the final mesh has a specific radius, regardless of Z aspect ratio, you can set
  % rescaleRadius = 1 below, to ensure a consistent spatial extent of the mesh.

radialRings = 24  ; % Number of meshed ring layers spanning radially from the origin to the outer boundary of the lightsail.  
 ...
 % above ~20 start to get really slow in simulations!
    radialRings = round(abs(radialRings));  %make sure it's an integer.  Do not change this line.
    radius=radialRings;  %duplicate as 'radius' for legacy reasons.  Do not change this line.
...
zAspectRatio = 1.40;%.2;  % a relative measure of the height of the sail strucutre produced by the non-flat node generators. 
...
curvatureMode = 'smooth'; % can be 'smooth' or 'faceted'.  Determines how the out-of-plane curvature is applied for 
...
xyMode = 'square'; % Can be 'hex', 'round', or 'square'
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Mapping is used to specify different optical and material properties for different regions of the sail, 
% analogous to picking patches of fabric for a quilt.
%
% By default, the entire sail is assumed to have uniform, constant, simple DEFAULT properties -- determined by 
% global values such as modulus (Ymod), thickness, reflectance (Irefl), etc. (see setupMaterialProperties).  
% If that's all you need, just set texture_map_modes = {} below, to disable additional mapping.
% 
% By using the below mapping functionalities, we can assign different properties to different regions of the sail.
% This is also how we can utilize more advanced optical models, such as angle-dependent (1D or 2D) optical properties.
% 
%
% 'Material' is used to describe most properties of the affected mesh region, including mechanical properties (modulus, thickness), 
% as well as basic/simple optical properties such as specular reflectance, absorption, and emissivity.  Note that some properites 
% might not be relevant for all simulation settings, for example, if you enable useSpecularTMM, then reflectance and absorption will 
% be calculated based on complex index of refraction and thickness instead of fixed absorption and reflectance values.  These are
% all part of the materials properties, but not all are used at all times.   (Most are though!)
% If you enable a more advanced model, e.g., use2DOpticalLUT, then optical forces and absorption are calculated using this alternate 
% model system, ignoring all optical properties specified in the material.
%
% 'Texture' is a separate identifier, similar to 'material,' but it is intended to specify properties with a specific orientation 
% within the mesh, such as nanophotonic patterning.  There might be cases where you want the same material to be assigned to multiple
% regions of texture, or vise versa, so I left them as separate mappable entities, but in most cases, each texture would be associated
% with a specific material.  The same texture can be mapped to multiple regions, each with different orientations.
%
% In general, 'material' is used to identify all properties a region of the lightsail, except those which require further 
% knowledge of the specific orientation of the material's structure in order to perform physics calculations.  For optical calculations,
% 'material' properties can be evaluated based solely on the incidence angle of light relative to the surface, whereas 'texture' properties
% are evaluated with knowledge of both the incidence angle as well as an orientation angle (akin to az/el angles).   
%
% So from an optics standpoint, 'materials' settings are used in calculations with scalar or 1D lookups/equations, with respect to angle.
% 'textures' settings are used in calculations with 2D lookups/equations, with respect to angle.
%
% You can augment physics calculations to vary in other dimensions, too, e.g., temperature dependence, by constructing appropriate lookup tables
% or equations to reference the local temperature in the physics calcs.  Unless you need 2D incidence angle for optics calcs, I recommend 
% constructing such functionality similar to the specularTMM model, which uses the 'material' to index a material-specific lookup list
% to retrieve the desired values.  
%
% Note that if I ever get around to adding polarization support... then the required lookup lists and tables will double -- one for s, one for p

%% Implementation
% Both 'Texture' and 'Material' are simple integer values, telling the program which data source to use to calculate the physics for that 
% region of the mesh.  
%
% 'Texture' mapping further requires you to specify a direction, in addition to the index number.
%
% Data sources include the MATERIALS array (see setupMaterialProperties), the LULs array (calculated from material properties automatically), 
% and the LUTs array, which is loaded separately from files to determine texture-specific properties.  
%
% By default, material and texture are 0 throughout the mesh.  This uses the default (non-indexed) properties in setupMaterialProperties
%
% There are three mapping mechanisms available, described below, with which you can assign values for material and texture throughout the mesh.
% You may use zero, one, two, or all three methods sequentially (but you can't repeat any of them multiple times).
% IMPORTANT: if you specify a texture or material value of zero during mapping, the mapper won't change the texture or material value for that 
% triangle, so it will retain whatever prior material/texture was present there.  You cannot use the mapper to command a region back to 0.
% This makes it easier to stack mapping methods to construct elaborate designs.  
%
% TEXTURE: 
%
% The array t_tex stores the texture value for each triangle.  The texture
% value is an integer, which describes a specific type of optical behavior. 
%
%  - a texture value of zero means the region will not use any texture-based
%    models, and will revert to whatever behavior would occur based on the
%    assigned material.
%
%  - a texture value of 1,2,... will specify which LUTs to use when calculating
%    texture-based physics.  (the LUTs must be separately and correctly configured.)  
%    It's OK to have unused textures in your mapping, but whatever textures you use
%    must have valid LUTs.
...
texture_map_modes = { 'pizza' };%, this specifies what mappers are used, and their order
...
% Any number of regions can be defined between a range of polar angles
% relative to the origin.  Note, you can't change the origin, sorry.
...
pizza_dirs = [ verti -horiz -verti horiz  verti]; % the texture directions for each pizza slice.  (these are vectors.) 
pizza_materials = [1 1 1 1 1]; % the material assignments for each pizza slize.  
...
% into an inner and an outer region.  The outer region settings are the 'crust':
...
pizza_crust_materials = ones(size(pizza_crust_texs));
%
...
%  during mesh generation based on membrane thickness and density.  That's somewhat crude, but it least gives us some 
%  control 
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% I added very redumentary support for a tethered payload.  It comprises a point mass located behind the sail, with 
% a fixed number of tethers to the perimeter of the sail.  Note that the tethers are modeled as simple linkages (springs)
% which prevents them from collapsing, but isn't a very good way to model a string!  I also had to choose the spring
% value somewhat arbitrarily, to keep its convergence timescale on par with the rest of the simulation.  So this is
% a very crude start to modelling tethered payloads, at best.  In particular, a real tether ligament would collapse 
% during spin-up or after acceleration.  If we want to investigate tethered payloads more rigorously, we'll need to
% properly model the tethers as compound linkages with physical bending stiffness.
usetether = 0;
tether_payload_distance = 0.25/zAspectRatio*radiusmm; %mm, initial z offset between the sail center vertex and the trailing payload
tether_payload_distance = tether_payload_distance + 45 * ( 250/250) * (radiusmm/500); % This is an approxiate correction for the distortion of spin stabilization, determined for an unburdened paraboloid with 500mm radius AR=0.48, spinning at 250 Hz, SiNx having around 0.1 g/m2 aperture-areal mass, for reverse raytracing studies 5/11/2022
tether_payload_mass = 1;%.0930*outersweepval1; %grams  5this is to vary the payload mass as a fraction of sail mass.
tether_k = 12 * sqrt(1e3); %sqrt(outersweepval1 / 0.001);  % tether spring constant...  This is kind of a guess

% Note: The location and number of tether attachments are determined in the tether code below.  Search for 'tethered_node_idxs'
...
%
% Do not make changes below unless you are trying to change the way the simulator works.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             e_w1(ne) = (2/3) * myarea / e_l0(ne); % Mike, why 2/3?  
    ...
    %%%        e_thk1(ne) = t_thick(nt); % assign thickness to edge
            ...
            if (e_t2(existingEdgeIdx) > 0)
                error(['Found more than 2 triangles at edge ' num2str(existingEdgeIdx) ]);
            else
...
e_na = e_na(1:ne+nte); % we keep room for payload tethers
e_nb = e_nb(1:ne+nte);
e_l0 = e_l0(1:ne+nte);
e_t1 = e_t1(1:ne);
e_t2 = e_t2(1:ne);
...
t_centerTris = n_ta(:,1);  %this is a list of all triangles bordering node #1, the center of the sail (zero padded)
t_centerTris = t_centerTris(t_centerTris>0);  %here we delete the padded zeros.  
...
%% initialize other triangle stuff.  These are default values, but might be changed by mapping!
                ...
                t_emis_fr = zeros(size(t_cx)) + Emissivity_fwd;
...
for ntm = 1:length(texture_map_modes)
    ...
    for nt = 1:length(t_na)
        ...
        %texture and which material should be applied to each triangle.  Forthermore, a direction vector specifies the in-plane
        %orientation of each texture region.  
        ...
        if isequal(texture_map_mode, 'pizza')
            ...
            setMaterialForTri(nt, pizza_materials(regi));
        ...
        elseif isequal(texture_map_mode, 'stuffedcrust')
            ...
            if (t_centroid_r(nt) >  pizza_crust_radii(regi) )
                ...
                setMaterialForTri(nt, pizza_crust_materials(regi));
            else
                if pizza_texs( regi) > 0
                    t_tex(nt) = pizza_texs( regi);
                    this_tex_dir = pizza_dirs(:,regi);
        ...
        if (et - lastET) > 1
            % Display progress of edge generation to user in %
            lastET = et;
            disp([ '    Sorry this part is very inefficiently coded, ' num2str(ntm) '/' num2str(length(texture_map_modes)) ': ' num2str(100*nt/length(t_na)) '%...']);
            % the slowness is that I'm using evalin() to change workspace variables in the setMaterialForTri() function... lame.
        end
    end
end
...
n_m = n_m + (t_m * M_t2n) ./ 3;  % The prior node mass is zero, except for the tethered payload if present.
...
%e_thk1 = zeros(1,numsailedges); % vector with thickness of each edge's first triangle %note: don't need to initilize this one, all edges have a t1
...
if any(t_nu > 0)
    usepoisson=1;
    e_nu1 = t_nu(e_t1(1:numsailedges));
    e_nu2 = zeros(1,numsailedges);
    e_nu2(~e_isouteredge) = t_nu(e_t2(~e_isouteredge)); 
end
...
%These are the non-poisson-corrected aspect lengths, which are still valid for thermal conductivity 
...
if (usepoisson)
    e_al1p = e_thk1.*e_w1 ./ e_l0 ./ (1 + e_nu1) + ...
                    (e_nu1 ./ ( 1 - e_nu1.^2)) .* (e_thk1.*e_w1b );
    e_al2p = e_thk2.*e_w2 ./ e_l0 ./ (1 + e_nu2) + ...
                    (e_nu2 ./ ( 1 - e_nu2.^2)) .* (e_thk2.*e_w2b );
    e_k = e_al1p .* t_ymod(e_t1) .* 1e-6;
    e_k(~e_isouteredge) = e_k(~e_isouteredge) + e_al2p(~e_isouteredge) .* t_ymod(e_t2(~e_isouteredge)) .*1e-6 ;
    e_k(e_isouteredge) = e_k(e_isouteredge)*1;  %multiply by 2? I'm on the fence as to whether or not this is accurate...  it reduces perimeter failure though!
    
    e_k0 = e_al1 .* t_ymod(e_t1) .* 1e-6;
    e_k0(~e_isouteredge) = e_k(~e_isouteredge) + e_al2(~e_isouteredge) .* t_ymod(e_t2(~e_isouteredge)) .*1e-6 ;
    e_k0(e_isouteredge) = e_k(e_isouteredge)*1;
    fprintf(1, 'Note:  Non-zero Poisson ratio in use.  Edge stiffness change:  %g mean    %g max      %d affected (%.1f%%)\n', ...
        mean(abs(e_k./e_k0 - 1)), max(abs(e_k./e_k0 - 1)), sum(e_k ~= e_k0), 100*(sum(e_k ~= e_k0)/length(e_k))  );
...
if usetether  % add in remaining vals for the tether ligaments
...
rmap_numtris = zeros(1,length(MATERIALS)+1);
inuse_materials = [];
used_materials = zeros(1,length(MATERIALS)+1);
nummat = 0;
rmap_names = { 'default' };

fprintf(['  Materials: '  sprintf('%12s  ', rmap_names{1} ) ]);
for mi = 1:(length(MATERIALS)+1);
    rmap_mass(mi) = sum(t_m(t_mat == (mi-1) ) );
    rmap_area(mi) = sum(t_a0(t_mat == (mi-1) ) );
    rmap_numtris(mi) = sum(t_mat == (mi-1) );
    if rmap_numtris(mi) > 0
        nummat = nummat+1;
        used_materials(nummat) = mi-1;
    end
    if (mi>1)  
        rmap_names{mi} = MATERIALS(mi-1).name; 
        fprintf('%12s  ', MATERIALS(mi-1).name);
    end
end
disp(' ');
disp(  ['       Mass: ' sprintf('%12g  ', rmap_mass)      ' g'  ] );
disp(  ['       Area: ' sprintf('%12g  ', rmap_area./1e6) ' m2' ] ); 
disp(  ['      #tris: ' sprintf('%12g  ', rmap_numtris)   '   ' ] ); 

disp(' ');
disp(  [' Mechanical: '  sprintf('%12s  ', rmap_names{:} )  ]);

disp(  ['      thick: ' sprintf('%12g  ', thickness*1e6)  sprintf('%12g  ', [MATERIALS.thickness].*1e6)  ' nm' ] );
disp(  ['       dens: ' sprintf('%12g  ', density*1000)  sprintf('%12g  ', [MATERIALS.density].*1000)  ' g/cc' ] );
disp(  ['       Ymod: ' sprintf('%12g  ', Ymod/1e9)  sprintf('%12g  ', [MATERIALS.Ymod]./1e9)  ' GPa' ] );
disp(  ['       Emod: ' sprintf('%12g  ', Emod/1e9)  sprintf('%12g  ', [MATERIALS.Emod]./1e9)  ' GPa' ] );
disp(  ['         nu: ' sprintf('%12g  ', nu)  sprintf('%12g  ', [MATERIALS.nu])  ' ' ] );
disp(' ');
disp(  ['    Optical: ' sprintf('%12s  ', rmap_names{:} )  ]);
if exist('useSpecularTMM','var')
    if useSpecularTMM
    nas = repmat({'-'},1,length(MATERIALS));
    disp(  ['        abs: ' sprintf('%12g  ', Iabs)  sprintf('%12s  ', nas{:})  ' ' ] );
    disp(  ['       refl: ' sprintf('%12g  ', Irefl)  sprintf('%12s  ', nas{:})  ' ' ] );
    disp(  ['          n: ' sprintf('%12s  ', '-')  sprintf('%12g  ', [MATERIALS.ior_n])  ' ' ] );
    disp(  ['          k: ' sprintf('%12s  ', '-')  sprintf('%12g  ', [MATERIALS.ior_k])  ' ' ] );
    clear nas;
    else
    disp(  ['        abs: ' sprintf('%12g  ', Iabs)  sprintf('%12g  ', [MATERIALS.Iabs])  ' ' ] );
    disp(  ['       refl: ' sprintf('%12g  ', Irefl)  sprintf('%12g  ',[MATERIALS.Irefl])  ' ' ] );
    end
else
    disp(  ['        abs: ' sprintf('%12g  ', Iabs)  sprintf('%12g  ', [MATERIALS.Iabs])  ' ' ] );
    disp(  ['       refl: ' sprintf('%12g  ', Irefl)  sprintf('%12g  ',[MATERIALS.Irefl])  ' ' ] );
...
disp(' ');
disp(  ['    Thermal: '  sprintf('%12s  ', rmap_names{:} )  ]);

disp(  ['       cond: ' sprintf('%12g  ', Thermcondmm*1000)  sprintf('%12g  ', [MATERIALS.Thermcondmm].*1000)  ' W/m/K' ] );
disp(  ['    heatCap: ' sprintf('%12g  ', heatCap)  sprintf('%12g  ', [MATERIALS.heatCap])  ' J/g/K' ] );
disp(  ['        CTE: ' sprintf('%12g  ', CTE)  sprintf('%12g  ', [MATERIALS.CTE]./1e9)  ' ppm/K' ] );
disp(  ['   failTemp: ' sprintf('%12g  ', failtemp)  sprintf('%12g  ', [MATERIALS.failtemp])  ' K' ] );
disp(  ['       emis: ' sprintf('%12g  ', Emissivity)  sprintf('%12g  ', [MATERIALS.Emissivity])  ' ' ] );
disp(  ['   emis_fwd: ' sprintf('%12g  ', Emissivity_fwd)  sprintf('%12g  ', [MATERIALS.Emissivity_fwd])  ' ' ] );
disp(' ');
...
%initial velocity:  %Note, you can't change initial velocity here, this is a derived value, we just initialize it correctly here:
meanvx = 0;
meanvy = 0;
meanvz = vz0;
...
for ne=1:length(e_na)
    if e_rigid(ne)
        plot3( [n_x(e_na(ne)) n_x(e_nb(ne))], [n_y(e_na(ne)) n_y(e_nb(ne))], [n_z(e_na(ne)) n_z(e_nb(ne))], 'k', 'LineWidth', 5);
    end
end
...
% this is a one-off calculation of the geometric radiative heat transfer coefficients from each triangle to another.  This is a rather superficial
% (but reasonable to first order) effort developed in June of 2023, in response to a reviewer's questions regarding whether we had considered
% radiative heat transfer in our calculation of lightsail heating (which we had not.)   Performing thise calculations allows us to quantify the
% first-order effects of radiative heat transfer.
%
% Approach: 
%  - During mesh generation, we calculate the view factors (including cosine-theta factors for both triangles, distance, and receiver area factor) for RHT
%  -   We also calculate the total occluded view factor for each triangle, which is used to reduce the "Free space" radiation calculated for normal radiative cooling 
%  - During simulation, we apply this static RHT "conductance" to transfer heat between triangles due to radiation, in addition to conduction
%      We also reduce the free-space radiative cooling by the occlusion factor mentioned above
%
% Shortcomings:
%  1.  The RHT conductance values are statically calculated, only at time of mesh generation.
%      This is done to make the impact on simulation time modest, since recalculating this for each step would be an O(n^2) complex problem, and we try to keep simulation steps of O(n) complexity, at least in terms of avoiding scripted iteration in matlab. 
%      Thus, changes to shape during evolution of the simulation do not impact RHT -- this is a major shortcoming for lightsails whose shape changes substantially!
%  2.  We assume that all triangles are in radiative contact -- thus, that the lightsail is a simple convex envelope, such as a cone or a paraboloid.
%      We do not consider occlusion of distant surfaces by nearer surfaces!
%  3.  We also do not consider second-order time-varying changes to the triangle area (due to strain or thermal expansion) 
%  4.  As with the existing radiative cooling model, we assume basic gray-body radiative transfer, with constant reciprocal emissivity, and lambertian angular distribution
%      Obviously, emissivity and absorption could vary greatly as a function of the local temperature, but we haven't yet implemented physics for
%      temperature-dependedt emissivity even for basic radiative cooling, so we're not going to figure it out now for this one-off cursory investigation!
%
% Summary:  The approach is highly simplified, but should provide good first-order quantitative insights into how radiative heat transfer affects temperature
%           distrubutions for the spin-stabilized paraboloid lightsails discussed in our present paper. 



if do_rht
    
    disp('Performing geometry calcs to determine static radiative heat transfer matrix...');
    % first, compute additional triangle geometry variables, which are normally skipped during meshgen...
    t_ev1 = [n_x; n_y; n_z]* M_nxyz2tev1;
    t_ev2 = [n_x; n_y; n_z]* M_nxyz2tev2;
    t_norms = cross( t_ev1, t_ev2 );  % not yet normalized
    t_a = max(vecnorm(t_norms), 1e-8); % this is 2*area
    t_norms = t_norms ./ t_a ; %ok now t_norms contains proper normal vectors (normalized)
    t_a = 0.5 .* t_a; % ok now area is correct

    
    t_rht_aemvfs = zeros(length(t_cx), length(t_cx)) ; % RHT conductance matrix (source area * view factor to target * product of emissivities), 
          % % this is going to be strictly triangular rather than symmetric, so it will act on radiative potential difference (T1^4 - T2^4) rather
          % than absolute temperature, and I'll have to figure out how to unfold the reciprocal heat flow when I write the time-domain code...
    tmp_rht_vfs = t_rht_aemvfs;
    %for nt=1:length(t_cx)-1
    for nt=1:length(t_cx) %-1
        %tmpvec_x = -t_cx(nt) + t_cx(nt+1:end);
        %tmpvec_y = -t_cy(nt) + t_cy(nt+1:end);
        %tmpvec_z = -t_cz(nt) + t_cz(nt+1:end);

        tmpvec_x = -t_cx(nt) + t_cx(1:end);
        tmpvec_y = -t_cy(nt) + t_cy(1:end);
        tmpvec_z = -t_cz(nt) + t_cz(1:end);

        tmpvec = [tmpvec_x; tmpvec_y; tmpvec_z]; % vector from local triangle centroid to all remaining triangle centroids
        tmp_r = vecnorm(tmpvec);  % distance (r) between triangle centroids (mm)
        tmpvec = tmpvec ./ tmp_r;  % (now unit directional vectors)

        %tmpcos1 = abs(dot(tmpvec, repmat(t_norms(:,nt),1,length(tmp_r))));
        %tmpcos2 = abs(dot(tmpvec, t_norms(:,nt+1:end)));

        tmpcos1 = abs(dot(tmpvec, repmat(t_norms(:,nt),1,length(tmp_r))));
        tmpcos2 = abs(dot(tmpvec, t_norms(:,1:end)));
        
        
        %tmp_vfs = tmpcos1 .* tmpcos2 ./ (pi * tmp_r.^2) .* t_a(nt+1:end);  % this is the view factor
        tmp_vfs = tmpcos1 .* tmpcos2 ./ (pi * tmp_r.^2) .* t_a(1:end);  % this is the view factor
        tmp_rht_vfs(nt,:) = tmp_vfs;
        tmp_rht_vfs(nt,nt) = 0;


        %t_rht_aemvfs(nt, nt+1:end) = t_a(nt) .* t_emis(nt) .* t_emis(nt+1:end) .* tmp_vfs;
        %t_rht_aemvfs(nt, nt+1:end) = t_a(nt) .* t_emis(nt) .* t_emis(nt+1:end) .* tmp_vfs(nt+1:end);
        t_rht_aemvfs(nt, :) = t_a(nt) .* t_emis(nt) .* t_emis(:) .* tmp_vfs(:);
        t_rht_aemvfs(nt,nt) = 0;
        %tmp_rht_vfs(nt,nt+1:end) = tmp_vfs;
    end
        
    t_rht_occl = sum(tmp_rht_vfs);
    t_rht_occl2 = sum(tmp_rht_vfs, 2);
   % t_rht_occl = sum(tmp_rht_vfs + tmp_rht_vfs'); %note that this is based on assumption of infiinentecimal differental areas, which becomes problematic for high-aspect-ratio meshes...
    t_rht_occl = min(t_rht_occl, 1);  % so we truncate the value to 1 to prevent violating physics, when we suffer from overestimating the occlusion.  
    %  I'm not totally sure I understand why we sometiems overestimate occlusion, but it only happens when we have really high aspect ratio
    %  geometry... it may also suggest that we are overestimating radiative heat transfer too!  But... for more typical aspect ratios, I get
    %  seemingly plausible occlusion factors, and the sum never exceeds 1.  So... there's something a bit wonky with the physics, but I think it's ok
   
    hRHTPreview = figure('Position', [1050   600    1800   600]);
    subplot(1,2,1)
    trisurf(TRI,n_x,n_y,n_z, t_rht_occl, 'LineStyle', 'none');
    axis equal
    hc = colorbar ;
    hc.Label.String = 'Hemispherical occlusion factor';
    hold on;
    set(gca,'fontsize',16)
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('z (mm)');
    title('Radiative heat transfer:  Free-space occlusion factor');

    subplot(1,2,2)
    trisurf(TRI,n_x,n_y,n_z, t_rht_occl2, 'LineStyle', 'none');
    axis equal
    hc = colorbar ;
    hc.Label.String = 'Hemisphererical factor ';
    hold on;
    set(gca,'fontsize',16)
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('z (mm)');
    title('Radiative heat transfer:  Illumination factor');
    ...
    disp('Completed RHT preparation!');
    figure(hMeshPreview);
end