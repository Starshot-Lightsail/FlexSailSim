...
function LUT = LoadLUT_v17(filepath, varname, LUTPsiStepDeg, LUTThetaStepDeg, showplots, need_rotate_data, assumed_constant_absorption)
...
if (size(bigtable,2)) == 4
    bigtable(:,5) = 0;  % col 5 would be for 'parallel' forces, which are zero for MEPS, so we just add all zeros here... 
end
if (size(bigtable,2)) == 5
    bigtable(:,6) = assumed_constant_absorption;  % col 6 is for absorption (absolute fraction of incident light).  Our MEPs sims don't calculate angular absorption, so we assume a constant value (passed in as parameter to this function)
end
...
LUTn = zeros(length(LUTPsis),length(LUTThetas));  %normal (propulsive) fraction (relative to 100% reflection surface at normal incidence)
LUTt = zeros(length(LUTPsis),length(LUTThetas));  %transverse (lateral) fraction
LUTp = zeros(length(LUTPsis),length(LUTThetas));  %parallel (lateral) fraction
LUTa = zeros(length(LUTPsis),length(LUTThetas));  %absorbed fraction
