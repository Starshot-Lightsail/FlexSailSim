...
% same as .ConnectivityList from delaunayTriangulation()
z     = peaks(n);             % sample function defined on a grid of the same dimenision
vertices = [x(:) y(:) z(:)];  % vertices stored as Nx3 matrix
% [n_x' n_y' n_z']
orig  = [0.25*n 0 2];         % ray's origin   %t_cxyz = [t_cx' t_cy' t_cz'];
dir   = [0.5 *n n 0];         % ray's direction   % t_refln'
vert1 = vertices(faces(:,1),:); % % t_naxyz = [n_x(t_na); n_y(t_na); n_z(t_na)]'
vert2 = vertices(faces(:,2),:); % t_nbxyz = [n_x(t_nb); n_y(t_nb); n_z(t_nb)]';
vert3 = vertices(faces(:,3),:); % t_ncxyz = [n_x(t_nc); n_y(t_nc); n_z(t_nc)]';
tic;
